  ...
  all_divs.forEach((e) => {
    ...
    e.addEventListener("click", function () {
      ...
      localStorage.setItem(
        `data${imgs.id}`,
        JSON.stringify(data.products[imgs.id])
      );
    });
  });
}
...
if (
  localStorage.getItem("logout1") === "false" &&
  localStorage.getItem("logout2") === "false"
) {
  localStorage.setItem("d-none", "block");
}
document.getElementById("logout-cart").style.display =
  localStorage.getItem("d-none");
